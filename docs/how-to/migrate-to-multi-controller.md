...
pv_prefix="DEVICE"))` with `EpicsCATransport()` plus passing the id as
the first positional argument to your Controller's constructor (or
relying on the launcher to pass the YAML entry's `id:` for you).
...
declare `id` as the first positional `__init__` parameter and forward
...
class MyController(Controller):
    def __init__(self, id: str, settings: MySettings):
        super().__init__(id, ...)      # forward to base

controller = MyController("DEVICE", settings)
```

When `launch()` instantiates Controllers from `fastcs.yaml`, it passes
each entry's `id:` as the first positional argument — so user code
never needs to set the id twice. A registered Controller class that
omits `id` from its `__init__` signature is rejected at startup with a
clear `LaunchError`.
