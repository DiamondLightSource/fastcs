...
controller = MyController(id="DEVICE")  # PV prefix for EPICS / route prefix for REST
...
The PV prefix is the controller's id, passed at construction
(`MyController(..., id="DEVICE")`) or by `launch()` from the YAML
entry's `id:` field.
...
controller = MyController(id="DEVICE")
