...
declarative spelling on its own, with nothing else to read past.
...
That single decorated method is a read-only ``AttrR[float]``: the datatype is
the return annotation, and the docstring's first line becomes the description a
transport shows next to the value. A value the device can be told as well as
asked is declared with ``AttrRW.declare`` and a ``@x.setter`` method::
...
The decorator names the class it builds, so ``greeting`` is an ``AttrRW[str]``
to a reader and to a type checker alike. The setter has a name of its own, as
PyTango's ``write_greeting`` does, so the two halves of one attribute are never
two declarations of one name.

The decorator's optional leading argument is a schedule, and its keyword
arguments are the attribute's metadata - so ``@AttrR.declare(Polled(period=0.2),
...
form uses; the decorator is sugar over those constructors rather than a second
way of doing it.
...
class HelloWorldController(Controller):
    ...
    async def greeting(self) -> str:
        """The word to greet with."""
        # A bare declaration is read once, when the controller connects, which
        # is what a value only changes because you changed it needs.
        return self._greeting
