    ...
    for entry in controllers_options:
        ...
        if registered.expects_options:
            ...
            controller = registered.cls(
                registered.options_type(**field_values), id=entry.id
            )
        else:
            controller = registered.cls(id=entry.id)
...
def _build_entry_model(controller_class: type[Controller]) -> type[BaseModel]:
    ...
    The user's ``__init__`` must declare ``id`` as a keyword-only parameter
    (the launcher passes ``id=entry.id`` at construction). Any further
    positional argument after ``self`` is treated as the options arg.
    """
    sig = inspect.signature(controller_class.__init__)
    spec = inspect.getfullargspec(controller_class.__init__)
    args = spec.args
    discriminator = _discriminator(controller_class)

    if "id" not in spec.kwonlyargs:
        raise LaunchError(
            f"'{controller_class.__name__}.__init__' must declare 'id' as a "
            f"keyword-only parameter (e.g. `*, id: str`) and forward it via "
            f"`super().__init__(id=id)`. Received {sig}."
        )
    ...
    if len(args) == 1:
        ...
        raise LaunchError(
            f"Expected no more than 2 positional arguments for "
            f"'{controller_class.__name__}.__init__' but received {len(args)} "
            f"as `{sig}`"
        )
