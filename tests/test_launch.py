...
class IsHinted(Controller):
    ...
    def __init__(self, id: str, arg: SomeConfig) -> None:
        super().__init__(id)


class ManyArgs(Controller):
    def __init__(self, id: str, arg: SomeConfig, too_many):
        super().__init__(id)


class OtherHinted(Controller):
    def __init__(self, id: str, arg: OtherConfig) -> None:
        super().__init__(id)
...
class MissingId(Controller):
    """Controller that forgot to declare `id` as the first positional param."""
...
def test_not_hinted_schema():
    error = (
        ...
        "(self, id: str, arg). Add a typehint for `arg`."
    )
...
def test_over_defined_schema():
    error = (
        "Expected no more than 3 positional arguments (self, id, options) for "
        "'ManyArgs.__init__' but received 4 as `(self, id: str, "
        "arg: tests.test_launch.SomeConfig, too_many)`"
    )
...
def test_missing_id_positional_rejected():
    """A Controller class registered with launch() must declare `id` as the
    first positional parameter so the launcher can pass id at construction."""
    with pytest.raises(LaunchError) as exc_info:
        launch(MissingId)
    assert "must declare 'id' as its first positional parameter" in str(exc_info.value)
