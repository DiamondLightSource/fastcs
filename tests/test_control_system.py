...
async def test_update_periods():
    ...
    class MyController(Controller):
        def __init__(self):
            super().__init__()
            self.update_once = AttrR(Int(), getter=get_once, poll_period=ONCE)
            self.update_quickly = AttrR(Int(), getter=get_quickly, poll_period=0.1)
            self.update_never = AttrR(Int(), getter=get_never, poll_period=None)
