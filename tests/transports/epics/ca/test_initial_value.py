...
class InitialValuesController(Controller):
    def __init__(self, *args, **kwargs) -> None:
        ...
        self.int_rw = AttrRW(int, initial_value=4)
        self.float_rw = AttrRW(float, initial_value=3.1)
        self.bool_rw = AttrRW(bool, initial_value=True)
        self.enum_rw = AttrRW(InitialEnum, initial_value=InitialEnum.B)
        self.str_rw = AttrRW(str, initial_value="initial")
        self.waveform_rw = AttrRW(
            Array1D[np.int64],
            initial_value=np.array(range(10), dtype=np.int64),
            shape=(10,),
        )
        self.int_r = AttrR(int, initial_value=5)
        self.float_r = AttrR(float, initial_value=4.1)
        self.bool_r = AttrR(bool, initial_value=False)
        self.enum_r = AttrR(InitialEnum, initial_value=InitialEnum.C)
        self.str_r = AttrR(str, initial_value="initial_r")
        self.waveform_r = AttrR(
            Array1D[np.int64],
            initial_value=np.array(range(10, 20), dtype=np.int64),
            shape=(10,),
        )
        self.waveform_w = AttrW(Array1D[np.int64], shape=(10,))
